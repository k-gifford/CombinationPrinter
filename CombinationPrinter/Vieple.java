...
public class Vieple {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter any characters (no spaces):");
        while(getting_characters){
            characterString = userin.nextLine();
            if(characterString.length() > 0){
                characters = new char[characterString.length()];
                for(int i = 0; i < characterString.length(); i++){
                    characters[i] = characterString.charAt(i);
                }
                getting_characters = false;
            }
            else{
                System.out.println("Enter four characters (no spaces):");
            }
        }
        System.out.println("Enter max string length of combinations (>0):");
        while(getting_maxlength){
            maxlength = userin.nextInt();
            if(maxlength > 0){
                getting_maxlength = false;
            }
            else{
                System.out.println("Enter max string length of combinations(>0):");   
            }
        ...
        System.out.println("All combinations of " + characterString + ":");        
        
        /** At first, I used a HashSet to remove duplicates but switched to 
         * TreeSet to maintain ordering for ease of verification
         */
        TreeSet<String> output = new TreeSet<>();   
        
        output = combinations(characters, output, "", maxlength);
     ...
     * @return 
     */
    public static TreeSet<String> combinations(char[] characters, TreeSet<String> output, String combo, int maxlength){
